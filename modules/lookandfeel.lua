...
-- Any window opening on a normal workspace un-floats the other windows
-- there, so new windows always tile in instead of stacking behind a float
hl.on("window.open", function(newWindow)
    local ws = newWindow.workspace
    if not ws or ws.special then return end

    for _, win in ipairs(hl.get_windows({ floating = true, workspace = ws.id })) do
        if win.address ~= newWindow.address and not win.pinned then
